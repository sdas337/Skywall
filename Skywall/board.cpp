...
bool Board::fiftyMoveCheck() {
	return boardStates.back().fiftyMoveCount >= 100;
}

bool Board::repeatedPositionCheck() {	// true for if the game a repeatedPosition
	int count = 2;
...
void Board::makeRawMove(Move move) {
...
void Board::undoRawMove(Move move) {
	int startSquare = move.getStartSquare();
	int targetSquare = move.getEndSquare();
	int pieceRemovalSquare = targetSquare;
	int flags = move.getFlag();
...
bool Board::sideInCheck(int player) {
...
char Board::prettyPiecePrint(int row, int col, int pieceAtLoc) {
...
void Board::setMoveFromString(Move& m, string currentMove) {
...
void Board::loadBoardFromFen(string fen) {
...
vector<Move> Board::generateLegalMovesV2(bool onlyCaptures) {	// Using faster check detection
...
uint64_t Board::zobristHashCalc() {
...
uint64_t Board::getAttackers(int square, uint64_t comboOB) {
...
uint64_t Board::generateRookMoves(int square, uint64_t blockerBitboard) {
...
uint64_t Board::generateBishopMoves(int square, uint64_t blockerBitboard) {
	uint64_t slidingMoveBitboard = 0ull;
...
void Board::generateZobristNumbers() {
...
void Board::calculateZobristHash() {
...
int Board::generateKingMovesV2(int square, vector<Move>& moves, int insertionIndex, int wantedPlayer) {
...
uint64_t Board::generatePawnAttacks(int square, int wantedPlayer) {
...
int Board::generatePawnMovesV2(int square, vector<Move>& moves, int insertionIndex, int wantedPlayer) {
	int movesGenerated = 0;
...
uint64_t Board::generateKnightMoves(int square, int safePlayer) {
	uint64_t legalMoveBoard = validKnightMoves[square];

	return legalMoveBoard & ~occupiedBoard[safePlayer];
}

int Board::generatePseudoLegalMovesV3(vector<Move>& listOfMoves, int wantedPlayer) {
...
void Board::generateAttacksV3(int wantedPlayer) {
	int square;
